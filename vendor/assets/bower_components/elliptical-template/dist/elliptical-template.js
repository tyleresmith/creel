...
(function (root, factory) {
    ...
    dust.onLoad = function (template, callback) {
        if(template.indexOf('.')===0){
            template=sanitizeDotNormalization(template);
            loadTemplateFromFolder(dust,template,callback);
        }else if (template.indexOf('.') > -1) loadTemplateFromViewsFolder(dust, template, callback);
        else {
    ...
    function sanitizeDotNormalization(template){
        if(template.indexOf('.')===0) template=string.trimFirstChar(template);
        return template;
    }
